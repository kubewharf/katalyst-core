...
func (n *NodeOvercommitment) nodeOvercommitRatio(nodeInfo *framework.NodeInfo) (CPUOvercommitRatio, memoryOvercommitRatio float64, err error) {
	...
	recommendCPUOvercommitAnnotation, ok := annotation[consts.NodeAnnotationRealtimeCPUOvercommitRatioKey]
	if ok {
		recommendCPUOvercommitRatio, err := strconv.ParseFloat(recommendCPUOvercommitAnnotation, 64)
		if err != nil {
			klog.Error(err)
		} else {
			if recommendCPUOvercommitRatio < CPUOvercommitRatio && recommendCPUOvercommitRatio >= 1 {
				CPUOvercommitRatio = recommendCPUOvercommitRatio
			}
		}
	}
	...
	recommendMemoryOvercommitAnnotation, ok := annotation[consts.NodeAnnotationRealtimeMemoryOvercommitRatioKey]
	if ok {
		recommendMemoryOvercommitRatio, err := strconv.ParseFloat(recommendMemoryOvercommitAnnotation, 64)
		if err != nil {
			klog.Error(err)
		} else {
			if recommendMemoryOvercommitRatio < memoryOvercommitRatio && recommendMemoryOvercommitRatio >= 1 {
				memoryOvercommitRatio = recommendMemoryOvercommitRatio
			}
		}
	}
