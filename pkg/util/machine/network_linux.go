...
func GetNicQueue2IrqWithQueueFilter(nicInfo *NicBasicInfo, queueFilter string, queueDelimeter string) (map[int]int, error) {
	...
	interrupts, err := procm.GetProcInterrupts()
	if err != nil {
		return nil, fmt.Errorf("failed to get proc interrupts, err %s", err)
	}

	queue2Irq := make(map[int]int)
	for irqName, interrupt := range interrupts {
		if !strings.Contains(interrupt.Devices, queueFilter) {
			continue
		}
		...
		queueCols := strings.Split(interrupt.Devices, queueDelimeter)
		if len(queueCols) < 2 {
			continue
		}
