...
func GetPodAffinityInfo(
	req *pluginapi.ResourceRequest,
	podAffinity *MicroTopologyPodAffnity,
	numaInfo []NumaInfo,
	cpuTopology *machine.CPUTopology,
) (*PreFilterState, error) {
	podAffinityInfo := RequiredPodAffinityInfo(podAffinity, req.Labels)
	...
	var socket2numaList = make(map[int][]int, cpuTopology.NumSockets)
	for i := 0; i < cpuTopology.NumSockets; i++ {
		numaList := cpuTopology.CPUDetails.NUMANodesInSockets(i).ToSliceInt()
		socket2numaList[i] = numaList
	}
	GetExistingAntiAffinityCounts(&state, socket2numaList)
	GetAntiAffinityCounts(&state, socket2numaList)
	GetAffinityCounts(&state, socket2numaList)

	return &state, nil
}

func PodAffinityFilter(
	state *PreFilterState,
	hints map[string]*pluginapi.ListOfTopologyHints,
) map[string]*pluginapi.ListOfTopologyHints {
	filterdHints := map[string]*pluginapi.ListOfTopologyHints{}

	for resource, topologyHints := range hints {
		var filterdTopologyHints []*pluginapi.TopologyHint

		for _, hint := range topologyHints.Hints {
			if HintPodAffinityFilter(state, hint) {
				filterdTopologyHints = append(filterdTopologyHints, hint)
			}
		}

		filterdHints[resource] = &pluginapi.ListOfTopologyHints{
			Hints: filterdTopologyHints,
		}
	}
...
// Calculate the number of existing pods that has anti-affinity seletor that match the "pod",
// and update the util.TopologyAffinityCount imformation
func GetExistingAntiAffinityCountsSerial(state *PreFilterState, socket2numaList map[int][]int) {
	numNUMA := len(state.NumaAffinityInfoList)
	topologyMaps := make([]TopologyAffinityCount, numNUMA)

	for numaID := 0; numaID < numNUMA; numaID++ {
		numaAffinity := state.NumaAffinityInfoList[numaID]
		numaInSameSocket := socket2numaList[numaAffinity.SocketID]
		topologyMaps[numaID] = matchNUMAAffinity(numaAffinity.AntiAffinityRequiredSelectors,
			state.PodAffinityInfo.Labels, numaAffinity.NumaID, numaInSameSocket)
...
// Calculate the number of existing pods that match the anti-affinity seletor of the "pod",
// and update the util.TopologyAffinityCount imformation
func GetAntiAffinityCountsSerial(state *PreFilterState, socket2numaList map[int][]int) {
	numNUMA := len(state.NumaAffinityInfoList)
	topologyMaps := make([]TopologyAffinityCount, numNUMA)
...
// Calculate the number of existing pods that match the affinity seletor of the "pod",
// and update the util.TopologyAffinityCount imformation
func GetAffinityCountsSerial(state *PreFilterState, socket2numaList map[int][]int) {
	numNUMA := len(state.NumaAffinityInfoList)
	topologyMaps := make([]TopologyAffinityCount, numNUMA)
