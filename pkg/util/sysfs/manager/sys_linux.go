...
func (m *manager) GetNicRxQueueRPS(nic string, queue int) (string, error) {
	nicSysDir := filepath.Join("/sys", ClassNetBasePath, nic)
...
func (m *manager) SetNicRxQueueRPS(nic string, queue int, destCPUSet machine.CPUSet) error {
	nicSysDir := filepath.Join("/sys", ClassNetBasePath, nic)
	queuePath := fmt.Sprintf("%s/queues/rx-%d", nicSysDir, queue)

	rpsConf := destCPUSet.String()
