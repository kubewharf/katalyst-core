...
// GetSummedReclaimedPercentage returns the sum of reclaimed percentages for
// consumerNames, capped at 100. Unknown consumers contribute 0 through
// GetReclaimedPercentage.
func GetSummedReclaimedPercentage(d *dynamic.Configuration, consumerNames []string) float64 {
	var summedPct float64
	for _, name := range consumerNames {
		summedPct += GetReclaimedPercentage(d, name)
	}
	if summedPct > 100 {
		return 100
	}
