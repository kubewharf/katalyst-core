...
func TestGetSummedReclaimedPercentage(t *testing.T) {
	t.Parallel()
	lockGlobalRegistry(t)
	resetRegistry()

	for _, name := range []string{"a", "b", "c"} {
		if err := RegisterNamedGenericConsumer(name, newTestConf("/"+name), nil); err != nil {
			t.Fatalf("RegisterNamedGenericConsumer(%q): unexpected error: %v", name, err)
		}
	}

	d := newDynamicWithPercentages(map[string]int{
		"a": 50,
		"b": 30,
		"c": 40,
	})
	...
	tests := []struct {
		...
		{
			name:      "unknown contributes zero",
			consumers: []string{"a", "unknown"},
			want:      50,
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := GetSummedReclaimedPercentage(d, tt.consumers); got != tt.want {
