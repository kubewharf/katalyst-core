...
func TestGetNetworkTopologyAllocationsAnnotations(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			currentAnnotations: func() map[string]string {
				...
				b, _ := json.Marshal(ta)
				return map[string]string{apiconsts.PodAnnotationTopologyAllocationKey: string(b)}
			}(),
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := getNetworkTopologyAllocationsAnnotations(tt.ai, tt.currentAnnotations)
