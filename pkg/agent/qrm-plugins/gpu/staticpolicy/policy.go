...
func NewStaticPolicy(agentCtx *agent.GenericContext, conf *config.Configuration,
	...
	gpuTopologyProvider, err := machine.NewGPUTopologyProvider(conf.GPUResourceNames)
	if err != nil {
		return false, agent.ComponentStub{}, fmt.Errorf("failed to initialize gpu topology provider: %v", err)
	}
	...
	stateImpl, err := state.NewCheckpointState(conf.QRMPluginsConfiguration, conf.GenericQRMPluginConfiguration.StateFileDirectory, GPUPluginStateFileName,
		gpuconsts.GPUResourcePluginPolicyNameStatic, gpuTopologyProvider, conf.SkipNetworkStateCorruption, wrappedEmitter)
	if err != nil {
