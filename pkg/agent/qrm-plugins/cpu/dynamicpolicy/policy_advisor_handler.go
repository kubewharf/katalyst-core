...
func (p *DynamicPolicy) checkAndApplyIfCgroupV1(calculationInfo *advisorsvc.CalculationInfo, resources *common.CgroupResources) error {
	if !common.CheckCgroup2UnifiedMode() {
		...
		currentBigGroupQuota, err := cgroupmgr.GetCPUWithRelativePath(calculationInfo.CgroupPath)
		if err != nil {
		...
		for _, podDir := range podDirs {
			...
			err = p.checkAllContainersQuota(pod, []int64{podRealQuota, resources.CpuQuota}, []int64{podCurrentQuota, currentBigGroupQuota.CpuQuota})
			if err != nil {
			...
			// then check pod
			err = p.applyCPUQuotaWithRelativePath(podRelativePath, podLimit, []int64{resources.CpuQuota}, []int64{currentBigGroupQuota.CpuQuota})
			if err != nil {
...
func (p *DynamicPolicy) applyCPUQuotaWithRelativePath(relativePath string, limit int64, parentGroupRealQuotas []int64, parentGroupCurrentQuotas []int64) error {
	...
	// Apply quota when current quota is unlimited (-1) and calculated quota is within parent group quotas
	if cpu.CpuQuota < 0 {
		canChange := true

		for _, parentGroupRealQuota := range parentGroupRealQuotas {
			if realQuota > parentGroupRealQuota {
				canChange = false
				break
			}
		}
		...
		for _, parentGroupCurrentQuota := range parentGroupCurrentQuotas {
			if parentGroupCurrentQuota != -1 && realQuota > parentGroupCurrentQuota {
				canChange = false
				break
			}
		}
		...
		if err != nil {
			return fmt.Errorf("ApplyCPUWithRelativePath %s to -1 with realQuota %v parentRealQuotas %v parentCurrentQuotas %v failed with error: %v", relativePath, realQuota, parentGroupRealQuotas, parentGroupCurrentQuotas, err)
		}
	}
...
func (p *DynamicPolicy) checkAllContainersQuota(pod *v1.Pod, parentGroupRealQuotas []int64, parentGroupCurrentQuotas []int64) error {
	...
	for relativePath, container := range allContainersRelativePathMap {
		...
		err := p.applyCPUQuotaWithRelativePath(relativePath, limit, parentGroupRealQuotas, parentGroupCurrentQuotas)
		if err != nil {
