		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedErr {
				...
				if tc.expectedCount > 0 {
					...
					if info1 := c1.WorkloadsEvictionInfo; info1 != nil {
						assert.Equal(t, int32(5), info1.Replicas)
						assert.NotZero(t, info1.LastEvictionTime)
					} else {
						t.Logf("pod-1 has no workload info, skipping details check")
					}
					...
					if info2 := c2.WorkloadsEvictionInfo; info2 != nil {
						assert.Zero(t, info2.LastEvictionTime)
					} else {
						t.Logf("pod-2 has no workload info, skipping details check")
					}
				}
			}
		})
...
func TestGetWorkloadEvictionInfo(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			stats, ok := result.StatsByWindow[float64(tc.expectedWindow)/3600]
			if tc.expectedCount > 0 {
				assert.True(t, ok)
