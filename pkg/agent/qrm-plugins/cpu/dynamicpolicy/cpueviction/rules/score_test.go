...
func TestScorer_Score(t *testing.T) {
	...
	tests := []struct {
		...
		fields: fields{
			enabledScorers: []string{ScorerNameDeploymentEvictionFrequency},
			scorerOptions:  EvictOptions{},
		},
		args: args{pods: makeCandidatePods()},
		...
		fields: fields{
			enabledScorers: []string{ScorerNameDeploymentEvictionFrequency, ScorerNameUsageGap},
			scorerOptions: EvictOptions{
				State: State{
					NumaOverStats: []NumaOverStat{{
						NumaID:         0,
...
func TestDeploymentEvictionFrequencyScorer(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			score := DeploymentEvictionFrequencyScorer(tt.pod, EvictOptions{})
