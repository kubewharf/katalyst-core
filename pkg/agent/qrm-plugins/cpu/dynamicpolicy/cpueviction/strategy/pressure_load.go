...
type CPUPressureLoadEviction struct {
	...
	metricsHistory map[string]history.Entries

	syncPeriod       time.Duration
	evictionPoolName string
	lastEvictionTime time.Time

	poolMetricCollectHandlers map[string]history.PoolMetricCollectHandler
...
func NewCPUPressureLoadEviction(emitter metrics.MetricEmitter, metaServer *metaserver.MetaServer,
	...
	plugin := &CPUPressureLoadEviction{
		...
		metricsHistory:   make(map[string]history.Entries),
	...
	plugin.poolMetricCollectHandlers = map[string]history.PoolMetricCollectHandler{
...
func (p *CPUPressureLoadEviction) collectMetrics(_ context.Context) {
	...
	if !dynamicConfig.EnableLoadEviction {
		p.metricsHistory = make(map[string]history.Entries)
		return
	}
	...
	for podUID, entry := range pod2Pool {
		for containerName, containerEntry := range entry {
			...
			for _, metricName := range handleMetrics.UnsortedList() {
				...
				snapshot := &history.MetricSnapshot{
...
func (p *CPUPressureLoadEviction) pushMetric(dynamicConfig *dynamic.Configuration,
	metricName, entryName, subEntryName string, snapshot *history.MetricSnapshot,
) {
	if p.metricsHistory[metricName] == nil {
		p.metricsHistory[metricName] = make(history.Entries)
	}

	if p.metricsHistory[metricName][entryName] == nil {
		p.metricsHistory[metricName][entryName] = make(history.SubEntries)
	}

	if p.metricsHistory[metricName][entryName][subEntryName] == nil {
		p.metricsHistory[metricName][entryName][subEntryName] = history.CreateMetricRing(dynamicConfig.LoadMetricRingSize)
	}

	p.metricsHistory[metricName][entryName][subEntryName].Push(snapshot)
}

func (p *CPUPressureLoadEviction) logPoolSnapShot(snapshot *history.MetricSnapshot, poolName string, withBound bool) {
	if snapshot == nil {
		return
	}
