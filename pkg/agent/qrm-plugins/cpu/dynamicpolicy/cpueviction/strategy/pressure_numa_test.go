...
func TestWithConsumerFilter(t *testing.T) {
	...
	tests := []struct {
		name     string
		pod      *v1.Pod
		expected bool // true表示pod应被保留，false表示pod应被过滤
	}{
		{
			...
			expected: true, // PodIsConsumer返回false，WithConsumerFilter应保留此Pod
		},
		{
			...
			expected: false, // PodIsConsumer返回true，WithConsumerFilter应过滤此Pod
		},
		{
			...
			expected: true, // 只要有一个容器包含PrimaryPort，PodIsConsumer返回false
		},
		{
			...
			expected: false, // 没有容器，PodIsConsumer返回true
		},
		{
			name:     "Keep nil pod",
			pod:      nil,
			expected: true, // PodIsConsumer返回false，WithConsumerFilter应保留nil
		},
	}
