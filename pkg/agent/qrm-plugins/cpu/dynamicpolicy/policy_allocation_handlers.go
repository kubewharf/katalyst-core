...
func (p *DynamicPolicy) allocationSidecarHandler(_ context.Context,
	...
	// the sidecar container also support VPA resize, update the allocation and machine state here
	allocationInfo := &state.AllocationInfo{
		PodUid:          req.PodUid,
		PodNamespace:    req.PodNamespace,
		PodName:         req.PodName,
	...
	if util.PodIsInVPAResizing(req) {
		...
		if originAllocationInfo == nil {
			general.Errorf("pod: %s/%s, container: %s request to cpu VPA resize alloation, but no origin allocation info, reject it",
				req.PodNamespace, req.PodName, req.ContainerName)
			return nil, fmt.Errorf("no origion cpu allocation info for VPA resize")
		}
...
func (p *DynamicPolicy) calcPoolResizeRequest(originAllocation, allocation *state.AllocationInfo, podEntries state.PodEntries) (string, int, float64, error) {
	...
	if poolResizeQuantity < 0 {
		// We don't need to adjust pool size in VPA scale in mode, wait advisor to adjust the pool size later.
		general.Infof("pod: %s/%s, container: %s request cpu VPA scale in (%.02f->%.02f)",
		...
		// We should adjust pool size in VPA scale out mode with resizeReqFloat64, and then wait advisor to adjust the pool size later.
		general.Infof("pod: %s/%s, container: %s request cpu VPA scale out (%.02f->%.02f)",
			allocation.PodNamespace, allocation.PodName, allocation.ContainerName, originPodAggregatedRequest, podAggregatedRequest)
	}

	// only support normal share and snb VPA resize now
	if state.CheckSharedNUMABinding(allocation) {
		// check snb numa migrate for VPA resize
		...
		if originTargetNumaID != targetNumaID {
			...
			general.Infof("pod %s/%s request VPA resize and it was migrate to a new NUMA (%d->%d), AggregatedPodRequest(%.02f)",
