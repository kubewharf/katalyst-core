...
func TestManagerImpl_UpdateCPUBurst(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "shared cores pods with no cpu burst policy means that it is set to 0",
			pods: []*v1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						UID: "test-pod",
		...
		{
			name: "dedicated cores pods with no cpu burst policy but admin qos config is true",
			pods: []*v1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						UID: "test-pod",
		...
		{
			name: "dedicated cores pods with no cpu burst policy but admin qos config is false",
			pods: []*v1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						UID: "test-pod",
		...
		{
			name: "dedicated cores pods with cpu burst policy closed sets cpu burst value to 0",
			pods: []*v1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						UID: "test-pod",
						Annotations: map[string]string{
							consts.PodAnnotationQoSLevelKey:       consts.PodAnnotationQoSLevelDedicatedCores,
							consts.PodAnnotationCPUEnhancementKey: `{"cpu_burst_policy":"closed", "cpu_burst_percent":"100"}`,
						},
					},
					Spec: v1.PodSpec{
						Containers: []v1.Container{
		...
		{
			name: "shared cores pods with cpu burst policy closed sets cpu burst value to 0",
			pods: []*v1.Pod{
				{
					ObjectMeta: metav1.ObjectMeta{
						UID: "test-pod",
	...
	for _, tt := range tests {
		mockey.PatchConvey(tt.name, t, func() {
			...
			if !tt.wantErr {
				assert.Equal(t, tt.wantResults, results, "test %s", tt.name)
			}
		})
	}
}
