...
func (p *DynamicPolicy) requestSufficientFilter() Filter {
	return func(request float64, numa int, machineState state.NUMANodeMap) bool {
		availableCPUQuantity := machineState[numa].GetAvailableCPUQuantity(p.reservedCPUs)
		if !cpuutil.CPUIsSufficient(request, availableCPUQuantity) {
			general.Warningf("numa_binding shared_cores container skip NUMA: %d available: %.3f request: %.3f",
				numa, availableCPUQuantity, request)
			return false
		}
		return true
	}
...
func (p *DynamicPolicy) usageFilter() Filter {
	return func(request float64, numa int, machineState state.NUMANodeMap) bool {
		usage, err := p.getNumaUsage(numa, machineState)
		if err != nil {
		...
		threshold, err := p.getNUMACpuUsageThreshold(metricThreshold, p.usageThresholdExpandFactor)
		if err != nil {
		...
		general.InfoS("usage filter", "res", res, "numa", numa, "request", request,
			"usage", usage, "usageAfterSched", usageAfterSched, "thresholdValue", thresholdValue)
		return res
	}
}

func (p *DynamicPolicy) hybridRequestScorer() Scorer {
	return Scorer{
		ScoreFunc: func(request float64, numa int, machineState state.NUMANodeMap) int {
			available := machineState[numa].GetAvailableCPUQuantity(p.reservedCPUs)
			allocatable := float64(machineState[numa].GetFilteredDefaultCPUSet(nil, nil).Difference(p.reservedCPUs).Size())
			allocated := allocatable - available + request

			thresholdValue := allocatable * p.requestScoreThreshold
			var score float64
			...
			general.InfoS("hybrid request scorer", "score", score, "numa", numa, "request", request, "allocated",
				...
				"requestScoreThreshold", p.requestScoreThreshold)
			return int(score)
		},
	}
}
...
// todo incomplete
func (p *DynamicPolicy) hybridUsageScorer() Scorer {
	return Scorer{
		ScoreFunc: func(request float64, numa int, machineState state.NUMANodeMap) int {
			usage, err := p.getNumaUsage(numa, machineState)
			if err != nil {
			...
			metricThreshold := p.dynamicConfig.GetDynamicConfiguration().MetricThresholdConfiguration
			threshold, err := p.getNUMACpuUsageThreshold(metricThreshold, 1)
			if err != nil {
				general.Errorf("cannot get threshold %v", err)
				return 0
			}
