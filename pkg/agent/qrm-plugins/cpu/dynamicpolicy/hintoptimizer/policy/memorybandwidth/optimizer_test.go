...
func TestMemoryBandwidthOptimizer_OptimizeHints(t *testing.T) {
		...
		},
		{
			...
			mocks: func(f *fields, a *args) {
				...
				f.state = func() state.State {
					...
					st, _ := state.NewCheckpointState(tmpDir, "test-state-ign-neg", "test", cpuTopology, false, func(_ *machine.CPUTopology, _ state.PodEntries) (state.NUMANodeMap, error) { return ms, nil }, dummyEmitter)
					return st
				}()
				mockey.Mock(spd.GetContainerMemoryBandwidthRequest).To(func(profilingManager spd.ServiceProfilingManager, podMeta metav1.ObjectMeta, cpuRequest int) (int, error) {
					switch podMeta.UID {
			...
			wantHints: &pluginapi.ListOfTopologyHints{
				Hints: []*pluginapi.TopologyHint{
					...
					{Nodes: []uint64{0}, Preferred: true},
					{Nodes: []uint64{1}, Preferred: true}, // Stays true because it's filtered out before re-evaluation based on negative.
				},
			},
		},
	}
	...
	for _, tt := range tests {
		mockey.PatchConvey(tt.name, t, func() {
			...
			assert.Equal(t, tt.wantHints, tt.args.hints)
		})
	}
}
