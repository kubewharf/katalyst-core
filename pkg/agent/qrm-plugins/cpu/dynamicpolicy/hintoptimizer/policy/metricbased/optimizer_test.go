	...
	}{
		{
			...
			fields: fields{
				conf: func() *config.Configuration {
					conf := config.NewConfiguration()
					conf.GetDynamicConfiguration().StrategyGroup.EnabledStrategies = []v1alpha1.Strategy{}
					return conf
				}(),
				emitter: metrics.DummyMetrics{},
			},
		...
		// - error in isNUMAOverThreshold
		{
			...
			fields: fields{
				conf: func() *config.Configuration {
					...
					conf.GetDynamicConfiguration().MetricThreshold = &metricthreshold.MetricThreshold{
		...
		},
		{
			...
			fields: fields{
				conf: func() *config.Configuration {
					...
					// Intentionally make MetricThreshold nil to cause an error in getNUMAMetricThresholdNameToValue
					conf.GetDynamicConfiguration().MetricThreshold = nil
					return conf
				}(),
				metaServer: func() *metaserver.MetaServer {
					cpuTopology, _ := machine.GenerateDummyCPUTopology(16, 1, 1)
					return &metaserver.MetaServer{ // MetaServer is needed for the check, but MetricThreshold is the key for failure
						MetaAgent: &agent.MetaAgent{
							KatalystMachineInfo: &machine.KatalystMachineInfo{
								CPUTopology: cpuTopology,
							},
...
func TestMetricBasedHintOptimizer_getNUMAMetricThresholdNameToValue(t *testing.T) {
	...
	confNoMetricThreshold.DynamicAgentConfiguration = dynamic.NewDynamicAgentConfiguration() // MetricThreshold is nil by default
	oNilMetricThreshold := &metricBasedHintOptimizer{
		conf:       confNoMetricThreshold,
		metaServer: metaServer,
	}
	confNoMetricThreshold.GetDynamicConfiguration().MetricThreshold = nil
...
func TestMetricBasedHintOptimizer_getNUMAMetricThreshold(t *testing.T) {
	...
	metricThresholdConf := &metricthreshold.MetricThreshold{
		Threshold: map[string]map[bool]map[string]float64{
			"Intel_CascadeLake": {
				false: { // Not a VM
					metricthreshold.NUMACPUUsageRatioThreshold: 0.8,
