...
func TestDynamicPolicy_applyCPUQuotaWithRelativePath(t *testing.T) {
	...
	mockey.PatchConvey("test applyCPUQuotaWithRelativePath", t, func() {
		...
		err := p.applyCPUQuotaWithRelativePath("test-relative-path-1", 400, []int64{res.CpuQuota}, []int64{mockBGCPU.CpuQuota})
		convey.So(err, convey.ShouldBeNil)
		convey.So(apply.Times(), convey.ShouldEqual, 1)

		err = p.applyCPUQuotaWithRelativePath("test-relative-path-2", 1000, []int64{res.CpuQuota}, []int64{mockBGCPU.CpuQuota})
		convey.So(err, convey.ShouldBeNil)
