...
func (n *numaFirst) sortAvailableNUMANodes() []int {
	numas := n.acc.details.NUMANodes().ToSliceNoSortInt()
...
func (n *numaFirst) sortAvailableSockets() []int {
	...
	for _, numa := range n.sortAvailableNUMANodes() {
		sockets := n.acc.details.SocketsInNUMANodes(numa).ToSliceNoSortInt()
...
func (n *numaFirst) sortAvailableDies() []int {
	var result []int
	for _, socket := range n.acc.sortAvailableSockets() {
		dies := n.acc.details.DiesInSockets(socket).ToSliceNoSortInt()
		n.acc.sort(dies, n.acc.details.CPUsInDies)
		result = append(result, dies...)
	}
	return result
}
...
func (n *numaFirst) sortAvailableCores() []int {
	var result []int
	for _, die := range n.acc.sortAvailableDies() {
		cores := n.acc.details.CoresInDies(die).ToSliceNoSortInt()
...
func (s *socketsFirst) sortAvailableNUMANodes() []int {
	...
	for _, socket := range s.sortAvailableSockets() {
		numas := s.acc.details.NUMANodesInSockets(socket).ToSliceNoSortInt()
...
func (s *socketsFirst) sortAvailableSockets() []int {
	sockets := s.acc.details.Sockets().ToSliceNoSortInt()
	s.acc.sort(sockets, s.acc.details.CPUsInSockets)
	return sockets
}

func (s *socketsFirst) sortAvailableDies() []int {
	var result []int
	for _, socket := range s.acc.sortAvailableSockets() {
		dies := s.acc.details.DiesInSockets(socket).ToSliceNoSortInt()
		s.acc.sort(dies, s.acc.details.CPUsInDies)
		result = append(result, dies...)
	}
	return result
}
...
func (s *socketsFirst) sortAvailableCores() []int {
	var result []int
	for _, die := range s.acc.sortAvailableDies() {
		cores := s.acc.details.CoresInDies(die).ToSliceNoSortInt()
...
func newCPUAccumulator(topo *machine.CPUTopology, availableCPUs machine.CPUSet, numCPUs int) *cpuAccumulator {
	acc := &cpuAccumulator{
		topo:          topo,
...
func (a *cpuAccumulator) isDieFree(dieID int) bool {
	return a.details.CPUsInDies(dieID).Size() == a.topo.CPUsPerDie()
}
...
func (a *cpuAccumulator) freeDies() []int {
	free := []int{}
	for _, die := range a.sortAvailableDies() {
		if a.isDieFree(die) {
			free = append(free, die)
		}
	}
	return free
}
...
// Sort all dies with free CPUs:
func (a *cpuAccumulator) sortAvailableDies() []int {
	return a.numaOrSocketsFirst.sortAvailableDies()
}
...
func (a *cpuAccumulator) sortAvailableCPUs() []int {
	var result []int
	for _, core := range a.sortAvailableCores() {
		cpus := a.details.CPUsInCores(core).ToSliceNoSortInt()
		sort.Ints(cpus)
...
func (a *cpuAccumulator) takeFullDies() {
	for _, die := range a.freeDies() {
		cpusInDie := a.topo.CPUDetails.CPUsInDies(die)
		if !a.needs(cpusInDie.Size()) {
			continue
		}
		klog.V(4).InfoS("takeFullDies: claiming die", "die", die)
		a.take(cpusInDie)
	}
}
...
func takeByTopologyNUMAPacked(topo *machine.CPUTopology, availableCPUs machine.CPUSet, numCPUs int) (machine.CPUSet, error) {
...
func takeByTopologyNUMADistributed(topo *machine.CPUTopology, availableCPUs machine.CPUSet, numCPUs int, cpuGroupSize int) (machine.CPUSet, error) {
	...
	for k := minNUMAs; k <= maxNUMAs; k++ {
		...
		if acc.numCPUsNeeded > 0 {
			return machine.NewCPUSet(), fmt.Errorf("accounting error, not enough CPUs allocated, remaining: %v", acc.numCPUsNeeded)
		}

		// Likewise, if we have allocated too many CPUs at this point, then something
		// went wrong in our accounting and we should error out.
		if acc.numCPUsNeeded < 0 {
			return machine.NewCPUSet(), fmt.Errorf("accounting error, too many CPUs allocated, remaining: %v", acc.numCPUsNeeded)
		}
...
// Support both intel and AMD, packed policy by default.
func takeByTopology(topo *machine.CPUTopology, availableCPUs machine.CPUSet, numCPUs int) (machine.CPUSet, error) {
	acc := newCPUAccumulator(topo, availableCPUs, numCPUs)
	if acc.isSatisfied() {
		return acc.result, nil
	}
	...
	// 2. Acquire whole dies, if available and the container requires at least
	//    a die's-worth of CPUs.
	acc.takeFullDies()
	if acc.isSatisfied() {
		return acc.result, nil
	}
...
// Take full core but only one thread is used
func takeByTopologyFreeCores(topo *machine.CPUTopology, availableCPUs machine.CPUSet, numCPUs int) (machine.CPUSet, error) {
	acc := newCPUAccumulator(topo, availableCPUs, numCPUs)
	if acc.isSatisfied() {
		return acc.result, nil
	}

	availableCores := acc.freeCores()
	if acc.numCPUsNeeded > len(availableCores) {
		return machine.NewCPUSet(), fmt.Errorf("not enough full cores available to satisfy request")
	}

	for _, coreID := range availableCores {
		klog.V(4).InfoS("takeRemainingCores: claiming core", "core", coreID)
		acc.take(machine.NewCPUSet(coreID))
		if acc.isSatisfied() {
			return acc.result, nil
		}
	}

	return machine.NewCPUSet(), fmt.Errorf("failed to allocate cores")
}
