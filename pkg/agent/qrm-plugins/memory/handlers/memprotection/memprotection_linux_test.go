...
func TestMemProtection(t *testing.T) {
			...
			StaticAgentConfiguration: &configagent.StaticAgentConfiguration{
				QRMPluginsConfiguration: &qrm.QRMPluginsConfiguration{
					MemoryQRMPluginConfig: &qrm.MemoryQRMPluginConfig{
						MemProtectionOptions: qrm.MemProtectionOptions{
							EnableSettingMemProtection:      true,
							MemProtectionQoSLevelConfigFile: "fake",
						},
					},
				},
			},
	...
	applyMemProtectionQoSLevelConfig(&coreconfig.Configuration{
		AgentConfiguration: &agent.AgentConfiguration{
			StaticAgentConfiguration: &configagent.StaticAgentConfiguration{
				QRMPluginsConfiguration: &qrm.QRMPluginsConfiguration{
					MemoryQRMPluginConfig: &qrm.MemoryQRMPluginConfig{
	...
	jsonContent := `{
		"mem_protection": {
			...
			"pod_explicitly_annotation_key": "MemcgProtectionValue",
			...
			StaticAgentConfiguration: &configagent.StaticAgentConfiguration{
				QRMPluginsConfiguration: &qrm.QRMPluginsConfiguration{
					MemoryQRMPluginConfig: &qrm.MemoryQRMPluginConfig{
						MemProtectionOptions: qrm.MemProtectionOptions{
							EnableSettingMemProtection:      true,
							MemProtectionQoSLevelConfigFile: absPath,
						},
					},
				},
			},
	...
	calculateMemProtection("fake", 10)
}
...
func TestCalculatedBestProtection(t *testing.T) {
	t.Parallel()

	result := calculatedBestProtection(1073741824.0, 536870912.0, 536870912.0)
	var expected uint64 = 0x22000000
	assert.Equal(t, expected, result, "Test alculatedBestProtection failed")

	result = calculatedBestProtection(1073741824.0, 5368709120.0, 536870912.0)
	expected = 0
	assert.Equal(t, expected, result, "Test alculatedBestProtection failed")
}
