...
func (p *DynamicPolicy) reclaimedCoresHintHandler(ctx context.Context,
	...
	if util.PodIsInVPAResizing(req) {
		general.Errorf("pod: %s/%s, container: %s request to memory VPA resize, but not support reclaimed cores",
...
func (p *DynamicPolicy) dedicatedCoresHintHandler(ctx context.Context,
	...
	if util.PodIsInVPAResizing(req) {
		general.Errorf("pod: %s/%s, container: %s request to memory VPA resize, but not support dedicated cores",
			req.PodNamespace, req.PodName, req.ContainerName)
		return nil, fmt.Errorf("not support VPA resize for didecated cores")
	}
...
func (p *DynamicPolicy) numaBindingHintHandler(_ context.Context,
	...
	if allocationInfo != nil {
		...
		hints = regenerateHints(uint64(podAggregatedRequest), allocationInfo, util.PodIsInVPAResizing(req))
		...
		general.Errorf("pod: %s/%s, container: %s request to VPA resize, but no origin allocation info",
			req.PodNamespace, req.PodName, req.ContainerName)
	...
	// check or regenerate hints for VPA mode
	if util.PodIsInVPAResizing(req) {
		if allocationInfo.NumaAllocationResult.Size() != 1 {
			general.Errorf("pod: %s/%s, container: %s is snb, but its numa size is %d",
				req.PodNamespace, req.PodName, req.ContainerName, allocationInfo.NumaAllocationResult.Size())
			return nil, fmt.Errorf("invalid hints for VPA pod")
		}
		...
		general.Infof("pod: %s/%s, main container: %s request to memory VPA resize (%d->%d)",
			req.PodNamespace, req.PodName, req.ContainerName, originPodAggregatedRequest, podAggregatedRequest)

		if originPodAggregatedRequest < uint64(podAggregatedRequest) && uint64(podAggregatedRequest)-originPodAggregatedRequest > nodeMemoryState.Free { // no left resource to scale out
			// TODO maybe support snb NUMA migrate VPA later
			isVpaMigratable := false
			if isVpaMigratable {
				general.Infof("pod: %s/%s, container: %s request to memory VPA resize (%d->%d), but no enough memory"+
				...
				// we just need to clear the main container of the pod, because the main container aggregated quantity
				// involve the whole pod (only for memory admit).
				resourcesMachineState, err = p.clearContainerAndRegenerateMachineState(req)
				if err != nil {
				...
				general.Infof("pod: %s/%s, container: %s request to memory VPA resize (%d->%d, diff: %d), but no enough memory(%d)",
					req.PodNamespace, req.PodName, req.ContainerName, originPodAggregatedRequest, podAggregatedRequest, uint64(podAggregatedRequest)-originPodAggregatedRequest, nodeMemoryState.Free)
...
func regenerateHints(reqInt uint64, allocationInfo *state.AllocationInfo, podIsInVpaResizing bool) map[string]*pluginapi.ListOfTopologyHints {
	hints := map[string]*pluginapi.ListOfTopologyHints{}

	allocatedInt := allocationInfo.AggregatedQuantity
	if allocatedInt < reqInt && !podIsInVpaResizing {
