...
func (r *resctrlHinter) HintResourceAllocation(podMeta commonstate.AllocationMeta, resourceAllocation *pluginapi.ResourceAllocation) {
	...
	if podMeta.QoSLevel == apiconsts.PodAnnotationQoSLevelSystemCores {
		// tweak the case of system qos
		resctrlGroup = commonstate.PoolNamePrefixSystem
	} else {
		resctrlGroup = commonstate.GetSpecifiedPoolName(podMeta.QoSLevel,
			r.getSharedSubgroupByPool(podMeta.Annotations[apiconsts.PodAnnotationCPUEnhancementCPUSet]))
	}
