...
func (r *resctrlHinter) getMonGroupsMaxCount() int {
	b, err := os.ReadFile(filepath.Join(resctrlRoot, "info/L3_MON/num_rmids"))
	if err != nil {
...
func (r *resctrlHinter) isMonGroupsOverLimit() bool {
	...
	subdirs, err := os.ReadDir(resctrlRoot)
	if err != nil {
		general.Errorf("mbm: check mon_groups: read root %s error: %v", resctrlRoot, err)
		return false
	}
	for _, subdir := range subdirs {
		...
		monGroupPath := filepath.Join(resctrlRoot, subdir.Name(), monGroups)
