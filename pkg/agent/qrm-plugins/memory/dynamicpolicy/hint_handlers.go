...
func (p *DynamicPolicy) dedicatedCoresWithNUMABindingHintHandler(_ context.Context,
	...
	if allocationInfo != nil {
		hints = rePackHints(uint64(reqInt), allocationInfo)

		// rePackHints failed, and we need to clear container record and re-calculate.
		if hints == nil {
...
func (p *DynamicPolicy) calculateHints(reqInt uint64,
	resourcesMachineState state.NUMANodeResourcesMap) (map[string]*pluginapi.ListOfTopologyHints, error) {
...
// rePackHints regenerates hints for container that'd already been allocated memory,
// and rePackHints will assemble hints based on already-existed AllocationInfo,
// without any calculation logics at all
func rePackHints(reqInt uint64, allocationInfo *state.AllocationInfo) map[string]*pluginapi.ListOfTopologyHints {
	hints := map[string]*pluginapi.ListOfTopologyHints{}
