...
func (p *DynamicPolicy) reclaimedCoresAllocationHandler(ctx context.Context,
	...
	if util.PodIsInVPAResizing(req) {
		...
		return nil, fmt.Errorf("not support VPA resize for reclaiemd cores")
	}
...
func (p *DynamicPolicy) numaBindingAllocationHandler(ctx context.Context,
	...
	if allocationInfo != nil {
		if allocationInfo.AggregatedQuantity >= uint64(podAggregatedRequest) && !util.PodIsInVPAResizing(req) {
		...
		general.Errorf("pod %s/%s, container: %s request to memory VPA, but no origin allocation info",
			req.PodNamespace, req.PodName, req.ContainerName)
	...
	if util.PodIsInVPAResizing(req) {
		...
		// regenerate memory state (without the current pod)
		var stateErr error
		memoryState, stateErr = state.GenerateMemoryStateFromPodEntries(p.state.GetMachineInfo(), podEntries, p.state.GetReservedMemory())
		if stateErr != nil {
			general.ErrorS(stateErr, "generateMemoryMachineStateByPodEntries failed",
