...
const minMBCapacity = 10_000

func isSupported(cpuVendor cpuid.Vendor) bool {
	// only AMD is supported
	return cpuVendor == cpuid.AMD
}
...
func NewGenericPolicy(agentCtx *agent.GenericContext, conf *config.Configuration,
	...
	if !isSupported(agentCtx.CPUInfo.CPUVendor) {
		general.Infof("[mbm] unsupported cpu arch %s", agentCtx.CPUInfo.CPUVendor)
	...
	general.Infof("[mbm] config: ccd cap kp %.3f, kq %.3f, groups %v", conf.CCDCapKp, conf.CCDCapKq, conf.CCDCapGroups)
	...
	monitor.SetMinActiveMB(conf.ActiveTrafficMBThreshold)
	reader.SetLocalIsVictimAndTotalIsAllRead(conf.LocalIsVictimAndTotalIsAllRead)

	domains, err := domain.NewDomainsByMachineInfo(agentCtx.KatalystMachineInfo, conf.MaxIncomingRemoteMB)
	if err != nil {
	...
	mbPlugin := newMBPlugin(agentCtx, conf,
		domains, metricsFetcher, planAllocator, agentCtx.EmitterPool)
