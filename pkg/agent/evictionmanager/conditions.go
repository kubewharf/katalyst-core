...
func isNotEvictionManagerCNRTaint(t *v1alpha1.Taint) bool {
	if t == nil {
		return false
	}
...
func (m *EvictionManger) getCNRTaintsFromConditions() []v1alpha1.Taint {
	...
	for conditionName, condition := range m.conditions {
		...
		taintKey := getTaintKeyFromConditionName(conditionName)
		for _, effect := range condition.Effects {
			qosLevelEffectString := strings.Split(effect, apiconsts.CNRConditionQoSLevelEffectSeparator)
			if len(qosLevelEffectString) != 2 {
				klog.Errorf("[eviction manager] invalid cnr taint effect: %s for condition: %s", effect, conditionName)
				continue
			}

			qosLevel := qosLevelEffectString[0]
			effect = qosLevelEffectString[1]
			if !validCNRTaintEffectQoSPrefixes.Has(qosLevel) {
...
func (m *EvictionManger) reportConditionsAsCNRTaints(ctx context.Context) {
	...
	notEvictionManagerCNRTaints := util.CNRTaintSetFilter(cnr.Spec.Taints, isNotEvictionManagerCNRTaint)
	// Add taints that are not managed by eviction manager.
	taints = append(taints, notEvictionManagerCNRTaints...)
	taintsToAdd, taintsToDel := util.CNRTaintSetDiff(taints, cnr.Spec.Taints)
	if len(taintsToAdd) != 0 || len(taintsToDel) != 0 {
