...
var (
	SyncHostPathIntv  = time.Minute
	CleanHostPathIntv = 10 * time.Minute
)
...
func (n *HostPathPodNotifier) Run(ctx context.Context) {
	go wait.Until(n.cleanHostPath, CleanHostPathIntv, ctx.Done())
	go wait.Until(n.createHostPath, SyncHostPathIntv, ctx.Done())
}
...
func (n *HostPathPodNotifier) createHostPath() {
	klog.Infof("[createHostPath] creating host path")
	podFilter := func(pod *v1.Pod) bool {
		...
		v, ok := pod.Annotations[apiconsts.PodAnnotationSoftEvictNotificationKey]
		if !ok {
			return false
		}
...
func (n *HostPathPodNotifier) cleanHostPath() {
	klog.Infof("[cleanHostPath] cleaning host path")
	...
	podList, err := n.metaServer.GetPodList(context.Background(), nil)
	if err != nil {
		klog.Errorf("get pod list failed: %s", err)
		return
	}

	existingPods := make(map[string]bool)
	for _, pod := range podList {
		existingPods[path.Base(n.genPodNotifyPath(pod))] = true
	}
	...
	for _, dir := range dirs {
		...
		if _, ok := existingPods[dirName]; !ok {
...
func notify(recorder events.EventRecorder, emitter metrics.MetricEmitter, pod *v1.Pod,
	...
	if pod == nil {
		return fmt.Errorf("[notifier] invalid pod")
	}
