...
func (p *topologyAdapterImpl) getZoneResources(allocatableResources *podresv1.AllocatableResourcesResponse) (map[util.ZoneNode]nodev1alpha1.Resources, error) {
	...
	zoneAllocatable, err = p.addNumaMemoryBandwidthResources(zoneAllocatable, helper.GetNumaAvgMBWAllocatableMap(p.metaServer.MetricsFetcher, p.metaServer.SiblingNumaAvgMBWCapacityMap, p.metaServer.SiblingNumaAvgMBWAllocatableRateMap))
	if err != nil {
		errList = append(errList, err)
	}
