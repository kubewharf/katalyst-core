...
func (p *nodeMetricsReporterPlugin) getPodUsage(pod *v1.Pod) (v1.ResourceList, map[int]v1.ResourceList, machine.CPUSet, bool, error) {
	...
	for _, container := range containers {
		...
		for numaID, metricCPUUsage := range metricContainerNUMACPUUsage {
			numa, err := strconv.Atoi(numaID)
			if err != nil {
				errList = append(errList, fmt.Errorf("failed to parse numaID, podUID=%v, containerName=%v, err=%v", pod.UID, container.ContainerName, err))
				continue
			}
			usages, ok := numaUsage[numa]
			if !ok {
