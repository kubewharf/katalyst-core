		...
		},
		{
			...
			args: args{
				metricSetter: func(f *metric.FakeMetricsFetcher, p *nodeMetricsReporterPlugin) {
					...
					for numaId := 0; numaId < 2; numaId++ {
						f.SetContainerNumaMetric("uid1", "container1", strconv.Itoa(numaId), consts.MetricsCPUUsageNUMAContainer, utilmetric.MetricData{Value: 1})
						f.SetContainerNumaMetric("uid1", "container1", strconv.Itoa(numaId), consts.MetricsMemAnonPerNumaContainer, utilmetric.MetricData{Value: 1 << 30})
					}
