...
func (mg *memoryGuard) calculateReclaimedMemoryLimitFor(numaID int, reclaimedCgroupPath string, zoneInfos []machine.NormalZoneInfo) (float64, error) {
	...
	highWatermark := numaTotal * watermarkScaleFactor.Value / float64(10000)
	if mg.conf.CriticalWatermarkName == plugins.HighWatermarkName {
		highWatermark = 2 * numaTotal * watermarkScaleFactor.Value / float64(10000)
	}
	...
	if found {
		...
		if mg.conf.CriticalWatermarkName == plugins.HighWatermarkName {
			highWatermark = float64(zoneInfo.High) * float64(mg.metaServer.KatalystMachineInfo.PageSize)
		} else {
			highWatermark = float64(zoneInfo.Low) * float64(mg.metaServer.KatalystMachineInfo.PageSize)
		}
	}
