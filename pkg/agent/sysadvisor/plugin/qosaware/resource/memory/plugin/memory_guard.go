...
func (mg *memoryGuard) Reconcile(status *types.MemoryPressureStatus) error {
	...
	memoryFree, err := mg.metaServer.GetNodeMetric(consts.MetricMemFreeSystem)
	if err != nil {
		return err
	}
	...
	buffer := memoryFree.Value - memoryTotal.Value*scaleFactor.Value/10000
	if buffer < 0 {
		buffer = 0
	}

	reclaimedClass := filepath.Base(mg.reclaimRelativeRootCgroupPath)
	reclaimGroupUsage, err := mg.metaServer.GetQoSClassMetric(reclaimedClass, consts.MetricMemUsageQoSClass)
	if err != nil {
		return err
	}

	general.Infof("memoryFree: %v, memoryTotal: %v, scaleFactor: %v, reclaimGroupUsage: %v",
		resource.NewQuantity(int64(memoryFree.Value), resource.BinarySI).String(),
		resource.NewQuantity(int64(memoryTotal.Value), resource.BinarySI).String(),
		scaleFactor.Value,
		resource.NewQuantity(int64(reclaimGroupUsage.Value), resource.BinarySI).String())

	mg.reclaimMemoryLimit.Store(int64(buffer + reclaimGroupUsage.Value))
