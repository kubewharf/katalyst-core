...
func NewLoadIsolator(conf *config.Configuration, emitter metrics.MetricEmitter,
...
func (l *LoadIsolator) GetIsolatedPods() []string {
	isolationResources := l.getIsolationResources()
	...
	for _, ci := range l.getSortedContainerInfo() {
		...
		if l.conf.IsolationDisabled || l.conf.IsolationDisabledPools.Has(ci.OwnerPoolName) {
			continue
		}
...
func (l *LoadIsolator) checkContainerIsolated(info *types.ContainerInfo, isolationResources map[string]*poolIsolationResources) (isolated bool) {
	defer func() {
		...
		if isolated && !isolationResources[info.OwnerPoolName].isolatedPods.Has(info.PodUID) {
			isolationResources[info.OwnerPoolName].isolatedPods.Insert(info.PodUID)
			isolationResources[info.OwnerPoolName].isolatedLimits += isolationResources[info.OwnerPoolName].podLimits[info.PodUID]
		}
	}()
...
func (l *LoadIsolator) checkContainerLoad(info *types.ContainerInfo) bool {
	load, err := l.metaServer.GetContainerMetric(info.PodUID, info.ContainerName, metric_consts.MetricLoad1MinContainer)
	if err != nil {
...
func (l *LoadIsolator) checkIsolationPoolLimit(info *types.ContainerInfo, isolationResources map[string]*poolIsolationResources) bool {
	poolResource := isolationResources[info.OwnerPoolName]
...
func (l *LoadIsolator) getIsolationResources() map[string]*poolIsolationResources {
	...
	l.metaCache.RangeContainer(func(podUID string, containerName string, ci *types.ContainerInfo) bool {
		if checkTargetContainer(ci) {
			// init for corresponding pool
			if _, ok := isolationResources[ci.OwnerPoolName]; !ok {
				maxRatio := l.conf.IsolatedMaxRatios
				if ratio, ratioOK := l.conf.IsolatedMaxPoolRatios[ci.OwnerPoolName]; ratioOK {
					maxRatio = ratio
				}

				isolationResources[ci.OwnerPoolName] = &poolIsolationResources{
			...
			if _, ok := isolationResources[ci.OwnerPoolName].podLimits[podUID]; !ok {
				isolationResources[ci.OwnerPoolName].podLimits[podUID] = 0.
			}

			isolationResources[ci.OwnerPoolName].totalLimits += ci.CPULimit
			isolationResources[ci.OwnerPoolName].podLimits[podUID] += ci.CPULimit
		}
		return true
	})
...
func checkTargetContainer(info *types.ContainerInfo) bool {
	return strings.HasPrefix(info.QoSLevel, consts.PodAnnotationQoSLevelSharedCores)
}
