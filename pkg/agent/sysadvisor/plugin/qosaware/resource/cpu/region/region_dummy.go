...
func (r *QoSRegionDummy) getCPUUsageRatio() (float64, error) {
	numaIDs := r.cpusetMems.ToSliceInt()
	if len(numaIDs) != 1 {
		return 0, fmt.Errorf("invalid dummy region binding numas: %v", r.cpusetMems.String())
	}
	numaID := numaIDs[0]
	...
	general.InfoS("dummy metrics",
		"regionName", r.name, "numaID", numaID, "cpuSet", cpuSet.String(), "usageRatio", usageRatio.Value)
