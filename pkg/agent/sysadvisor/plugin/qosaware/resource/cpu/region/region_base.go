...
func (r *QoSRegionBase) fetchGetter(indicatorName workloadv1alpha1.ServiceSystemIndicatorName) (indicatorTargetGetter, string) {
	metricThresholdEnabled, _ := strategygroup.IsStrategyEnabledForNode(coreconsts.StrategyNameMetricThreshold, true, r.conf)
	if metricThresholdEnabled && indicatorName == workloadv1alpha1.ServiceSystemIndicatorNameCPUUsageRatio {
		return r.indicatorTargetGetters[string(pkgconsts.IndicatorTargetGetterMetricThreshold)],
			string(pkgconsts.IndicatorTargetGetterMetricThreshold)
	}
