...
func (p *PolicyCanonical) estimateContainerCPUUsage(ci *types.ContainerInfo) (float64, error) {
	return helper.EstimateContainerResourceUsage(ci, v1.ResourceCPU, p.metaReader, p.essentials.EnableReclaim)
}
...
func (p *PolicyCanonical) estimationCPUUsage() (cpuEstimation float64, containerCnt uint, err error) {
	for podUID, containerSet := range p.podSet {
		for containerName := range containerSet {
			...
			containerEstimation, err := p.estimateContainerCPUUsage(ci)
			if err != nil {
				return 0, 0, err
			}
