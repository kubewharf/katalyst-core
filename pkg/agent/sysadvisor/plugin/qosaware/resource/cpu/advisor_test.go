...
func TestAdvisorUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// Make empty NUMA region (NUMA reclaim cgroup) have non-zero usage so the indicator is valid.
			cgroupMetricItems: []cgroupMetricItem{
				{
					cgroupPath: "/kubepods/besteffort-0",
					name:       pkgconsts.MetricCPUUsageCgroup,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			conf.EnableEmptyNUMARegion = tt.enableEmptyNUMARegion
