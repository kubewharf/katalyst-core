...
func updateCPUUsageIndicatorOffset(_ types.PodSet, _ float64,
	...
	ret, _, err := latencyregression.GetLatencyRegressionPredictResult(metaReader)
	if err != nil {
	...
	if err != nil {
		klog.Warningf("borwein v2 strategy is not enabled %v", err)
		return 0, err
	}

	targetOffset := latencyregression.MatchSlotOffset(predictAvg, strategy.StrategySlots, borweinParameter)

	offsetMin := borweinParameter.OffsetMin
	if specialOffset, isIn := latencyregression.InSpecialTime(strategy.StrategySpecialTime); isIn {
...
func fetchBorweinV2Strategy(conf *config.Configuration) (*latencyregression.BorweinStrategy, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get %v strategy param error: %v", strategyName, err)
	}
	if !enabled {
		return nil, fmt.Errorf("borwein strategy is not enabled")
	}
	// unmarshall
	...
	klog.Infof("borwein v2 strategy: %+v", strategy)
	return &strategy, nil
}
