...
func TestMemoryHeadroomReporting_GetFeatureGate(t *testing.T) {
	t.Parallel()
	...
	e := &MemoryHeadroomReporting{}

	t.Run("enabled returns feature gate", func(t *testing.T) {
		t.Parallel()
		fg := e.GetFeatureGate(newConf(true))
		if assert.NotNil(t, fg) {
			assert.Equal(t, NegotiationFeatureGateMemoryHeadroomReporting, fg.Name)
			assert.Equal(t, finders.FeatureGateTypeMemory, fg.Type)
