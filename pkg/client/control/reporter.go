...
func (d DummyReporter) Run(_ context.Context) {}
...
func (c *genericReporterPlugin) Run(ctx context.Context) {
	if err := c.startFunc(); err != nil {
		klog.Fatalf("start %v failed: %v", c.name, err)
	}
...
func (c *genericReporterPlugin) ReportContents(ctx context.Context, contents []*v1alpha1.ReportContent, fastPush bool) error {
	...
	if fastPush && apiequality.Semantic.DeepEqual(lastResp, resp) && c.notifierCh != nil {
		for {
			select {
			case c.notifierCh <- struct{}{}:
