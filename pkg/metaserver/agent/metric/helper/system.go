...
func GetNumaAvgMBWAllocatableMap(metricsFetcher types.MetricsFetcher, siblingNumaInfo *machine.SiblingNumaInfo) map[int]int64 {
	...
	for numaID, defaultCapacity := range siblingNumaInfo.SiblingNumaAvgMBWCapacityMap {
		metric, err := metricsFetcher.GetNumaMetric(numaID, consts.MetricMemBandwidthTheoryNuma)
		if err != nil || metric.Value == 0 {
