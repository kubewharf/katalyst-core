...
func Test_getNumaAndMaxBandwidth(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			cpuCodeName: "Zen4",
			expectBW:    consts.SocketBandwidthMap[consts.PlatformGeona] / uint64(consts.CcdCountMap[consts.PlatformGeona]),
		},
		{
			name:        "Zen3 cpu code (Milan)",
			cpuCodeName: "Zen3",
			expectBW:    consts.SocketBandwidthMap[consts.PlatformMilan] / uint64(consts.CcdCountMap[consts.PlatformMilan]),
		},
		{
			name:        "Zen2 cpu code (Rome)",
			cpuCodeName: "Zen2",
			expectBW:    consts.SocketBandwidthMap[consts.PlatformRome] / uint64(consts.CcdCountMap[consts.PlatformRome]),
		},
		{
			name:        "Rapids cpu code",
			cpuCodeName: "Rapids",
			expectBW:    consts.SocketBandwidthMap[consts.PlatformRapids] / uint64(consts.CcdCountMap[consts.PlatformRapids]),
		},
		{
			name:        "Lake cpu code",
			cpuCodeName: "Lake",
			expectBW:    consts.SocketBandwidthMap[consts.PlatformLake] / uint64(consts.CcdCountMap[consts.PlatformLake]),
		},
		{
...
func Test_aggregateNUMABytesPS(t *testing.T) {
	t.Parallel()
	l3BytesPS := map[int]malachitetypes.BytesPS{
	...
	tests := []struct {
		name      string
		input     map[int]malachitetypes.BytesPS
		cpuCode   string
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			result := aggregateNUMABytesPS(tc.input)
