...
func (m *MalachiteMetricsProvisioner) processSystemNUMAComputeData(systemComputeData *malachitetypes.SystemComputeData) {
	...
	l3CacheBandwidthStats := make(map[int]malachitetypes.BytesPS)
	oldL3CacheBandwidthStats, ok := m.metricStore.GetByStringIndex(consts.MetricL3MbmTotalPs).(map[int]malachitetypes.BytesPS)
	if !ok {
	...
	for _, l3Cache := range curL3Mon.L3Mon {
		...
		l3CacheBandwidthStats[l3Cache.ID] = malachitetypes.BytesPS{
			NumaID:           numaID,
	...
	numaBandwidthStats := aggregateNUMABytesPS(l3CacheBandwidthStats)
