...
func (m *MalachiteMetricsProvisioner) getCgroupPaths() []string {
	...
	offlineQosLevels := GetOfflineBestEffortQosLevels()
	general.Infof("offlineQosLevels: %v", offlineQosLevels)
	reclaimPath := common.GetReclaimRelativeRootCgroupPath(m.baseConf.ReclaimRelativeRootCgroupPath, 1)
	general.Infof("reclaimPath: %v", reclaimPath)

	for _, qosLevel := range offlineQosLevels {
		cgroupPaths = append(cgroupPaths, filepath.Join(common.CgroupFsRootPath, qosLevel))
	}
...
func GetOfflineBestEffortQosLevels() (qosList []string) {
	var KubePodCgroupPath string
	v1Exist := general.IsPathExists(filepath.Join(common.CgroupFSMountPoint, common.CgroupCpuacct))
	if v1Exist {
		KubePodCgroupPath = filepath.Join(common.CgroupFSMountPoint, common.CgroupCpuacct, common.CgroupFsRootPath)
	} else {
		KubePodCgroupPath = filepath.Join(common.CgroupFSMountPoint, common.CgroupFsRootPath)
	}
	fInfos, err := ioutil.ReadDir(KubePodCgroupPath)
	if err != nil {
		klog.ErrorS(err, "Error read dir", "path", KubePodCgroupPath)
		return
	}
	for _, fi := range fInfos {
		...
		if strings.Contains(fi.Name(), common.OfflineBECgroupPrefix) {
			qosList = append(qosList, fi.Name())
		}
	}
	return
}
...
func (m *MalachiteMetricsProvisioner) processSystemMemoryData(systemMemoryData *malachitetypes.SystemMemoryData) {
	...
	var metricLastUpdateTime, _ = m.metricStore.GetNodeMetric(consts.MetricMemUpdateTimeSystem)
	...
	m.processNodeMemStat(systemMemoryData, metricLastUpdateTime.Value)
