...
func getCPUPlatform(cpuCode string) (string, error) {
	switch {
	case strings.Contains(cpuCode, "Zen4"):
		return consts.PlatformGeona, nil
	case strings.Contains(cpuCode, "Zen3"):
		return consts.PlatformMilan, nil
	case strings.Contains(cpuCode, "Zen2"):
		return consts.PlatformRome, nil
	case strings.Contains(cpuCode, "Rapids"):
		return consts.PlatformRapids, nil
	case strings.Contains(cpuCode, "Lake"):
...
func getCCDMaxMemBandwidth(cpuCode string) uint64 {
	platform, err := getCPUPlatform(cpuCode)
	if err != nil {
		return 0
	}

	ccdCount, ok := consts.CcdCountMap[platform]
	if !ok || ccdCount == 0 {
		ccdCount = 1
	}

	socketBW, ok := consts.SocketBandwidthMap[platform]
	if !ok {
		socketBW = consts.MaxMBGBps // default
	}
...
func aggregateNUMABytesPS(l3BytesPS map[int]types.BytesPS, cpuCodeName string) map[int]types.BytesPS {
	numaBytesPS := make(map[int]types.BytesPS)
	for _, mbmStat := range l3BytesPS {
		numaBandwidthStats, exist := numaBytesPS[mbmStat.NumaID]
		if !exist {
			numaBandwidthStats = types.BytesPS{NumaID: mbmStat.NumaID}
		}
