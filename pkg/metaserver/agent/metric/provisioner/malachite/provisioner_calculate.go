...
// processNodeMemStat handles memory statistics in a period while,
// and it will need the previously collected data to do this
func (m *MalachiteMetricsProvisioner) processNodeMemStat(systemMemoryData *types.SystemMemoryData, lastUpdateTimeInSec float64) {
	var (
		...
		lastMemKswapdSteal                  = uint64(prevMemKswapdStealMetric.Value)
		lastMemVmStatPgStealDirect          = uint64(prevMemVmStatPgStealDirectMetric.Value)
		lastMemVmStatPgScanKswapd           = uint64(prevMemVmStatPgScanKswapdMetric.Value)
		lastMemVmStatPgScanDirect           = uint64(prevMemVmStatPgScanDirectMetric.Value)
		curUpdateTime                       = systemMemoryData.UpdateTime
	)
	mem := systemMemoryData.System
	m.setNodeRateMetric(consts.MetricMemKswapdstealRateSystem, func() float64 {
		return float64(uint64CounterDelta(lastMemKswapdSteal, mem.VmstatPgstealKswapd))
	}, int64(lastUpdateTimeInSec), curUpdateTime)
	m.setNodeRateMetric(consts.MetricMemVmStatPgStealDirectRateSystem, func() float64 {
		return float64(uint64CounterDelta(lastMemVmStatPgStealDirect, mem.VMStatPgStealDirect))
	}, int64(lastUpdateTimeInSec), curUpdateTime)
	m.setNodeRateMetric(consts.MetricMemVmStatPgScanKswapdRateSystem, func() float64 {
		return float64(uint64CounterDelta(lastMemVmStatPgScanKswapd, mem.VMStatPgScanKswapd))
	}, int64(lastUpdateTimeInSec), curUpdateTime)
	m.setNodeRateMetric(consts.MetricMemVmStatPgScanDirectRateSystem, func() float64 {
		return float64(uint64CounterDelta(lastMemVmStatPgScanDirect, mem.VMStatPgScanDirect))
	}, int64(lastUpdateTimeInSec), curUpdateTime)
...
func (m *MalachiteMetricsProvisioner) setNodeRateMetric(targetMetricName string, deltaValueFunc func() float64, lastUpdateTime, curUpdateTime int64) {
	timeDeltaInSec := curUpdateTime - lastUpdateTime
	if lastUpdateTime == 0 || timeDeltaInSec <= 0 {
		// Return directly when the following situations happen:
		// 1. lastUpdateTime == 0, which means no previous data.
	...
	updateTime := time.Unix(curUpdateTime, 0)
	m.metricStore.SetNodeMetric(targetMetricName,
		metric.MetricData{Value: deltaValueFunc() / float64(timeDeltaInSec), Time: &updateTime})
}
