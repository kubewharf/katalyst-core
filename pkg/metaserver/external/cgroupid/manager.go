...
// Run starts a CgroupIDManager
func (m *CgroupIDManager) Run(ctx context.Context) {
...
func (m *CgroupIDManager) GetCgroupIDForContainer(podUID, containerID string) (uint64, error) {
...
func (m *CgroupIDManager) ListCgroupIDsForPod(podUID string) ([]uint64, error) {
	m.RLock()
	defer m.RUnlock()
...
func (m *CgroupIDManager) reconcileCgroupIDMap(ctx context.Context) {
	podList, err := m.GetPodList(ctx, nil)
	if err != nil {
		klog.Errorf("[CgroupIDManager.reconcileCgroupIDMap] get pod list failed, err: %v", err)
		return
	}
...
func (m *CgroupIDManager) addAbsentCgroupIDsToCache(absentContainers map[string]sets.String) {
	klog.V(4).Infof("[CgroupIDManager] exec addAbsentCgroupIDsToCache")
	...
	for podUID, absentContainerSet := range absentContainers {
		for {
			...
			if err != nil {
				klog.Errorf("[CgroupIDManager.addAbsentCgroupIDsToCache] get cgroup id failed, pod: %s, container: %s, err: %v",
					podUID, containerID, err)
				continue
			}

			klog.Infof("[CgroupIDManager.addAbsentCgroupIDsToCache] add absent cgroup id to cache, "+
...
func (m *CgroupIDManager) getAbsentContainers(podList []*v1.Pod) map[string]sets.String {
	...
	for _, pod := range podList {
		...
		for _, container := range pod.Spec.Containers {
			...
			if err != nil {
				klog.Errorf("[CgroupIDManager.addNewCgroupIDsToCache] get container id failed, pod: %s, container: %s, err: %v",
...
func (m *CgroupIDManager) clearResidualPodsInCache(podList []*v1.Pod) {
	klog.V(4).Infof("[CgroupIDManager] exec clearResidualPodsInCache")
	...
	for podUID := range m.podCgroupIDCache {
		if !podSet.Has(podUID) && !residualSet[podUID] {
			...
			klog.V(4).Infof("[CgroupIDManager.clearResidualPodsInCache] found pod: %s with cache but doesn't show up in pod watcher, hit count: %d", podUID, m.residualHitMap[podUID])
		}
	}
	...
	for podUID, hitCount := range m.residualHitMap {
		if !residualSet[podUID] {
			klog.V(4).Infof("[CgroupIDManager.clearResidualPodsInCache] already found pod: %s in pod watcher or its cache is cleared, delete it from residualHitMap", podUID)
	...
	if podsToDelete.Len() > 0 {
		for {
			...
			klog.Infof("[CgroupIDManager.clearResidualPodsInCache] clear residual pod: %s in cache", podUID)
			delete(m.podCgroupIDCache, podUID)
		}
	}
}

func (m *CgroupIDManager) getCgroupIDFromCache(podUID, containerID string) (uint64, bool) {
	m.RLock()
	defer m.RUnlock()
...
func (m *CgroupIDManager) getCgroupIDFromSystem(podUID, containerID string) (uint64, error) {
...
func (m *CgroupIDManager) setCgroupID(podUID, containerID string, cgroupID uint64) {
	m.Lock()
	defer m.Unlock()
