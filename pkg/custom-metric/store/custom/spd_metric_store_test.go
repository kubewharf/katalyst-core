...
func TestIHPAMetricStore(t *testing.T) {
	...
	t.Run("GetMetric", func(t *testing.T) {
		...
		expected := []types.Metric{&types.SeriesMetric{MetricMetaImp: types.MetricMetaImp{Name: "spd_agg_metrics"}, Values: []*types.SeriesItem{types.NewInternalItem(float64(200), now.Add(-time.Minute).UnixMilli())}}}
