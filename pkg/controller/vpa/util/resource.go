...
func GetVPAResourceStatusWithCurrent(vpa *apis.KatalystVerticalPodAutoscaler, pods []*core.Pod) ([]apis.PodResources, []apis.ContainerResources, error) {
	...
	updateContainerResourcesCurrent := func(targetResourceNames map[consts.ContainerName][]core.ResourceName, containerName consts.ContainerName,
		target core.ResourceList, current *core.ResourceList, specResource core.ResourceList) {
			...
			func(pods []*core.Pod) {
				for _, pod := range pods {
					...
					for _, container := range pod.Spec.Containers {
						...
						podResources[podContainerName].Requests.Current =
							getPodContainerResourcesCurrent(podResources[podContainerName].Requests.Target, container.Resources.Requests)
						return
					}
				}
			}(pods)
			...
			func(pods []*core.Pod) {
				for _, pod := range pods {
					...
					for _, container := range pod.Spec.Containers {
						...
						podResources[podContainerName].Limits.Current =
							getPodContainerResourcesCurrent(podResources[podContainerName].Limits.Target, container.Resources.Limits)
						return
					}
				}
			}(pods)
