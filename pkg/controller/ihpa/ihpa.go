...
func (p *IHPAController) emitMetrics() {
	...
	for _, ihpa := range ihpaList {
		_ = p.metricsEmitter.StoreInt64(metricsNameIHPAReplicasMetric, int64(ihpa.Status.DesiredReplicas), katalystmetrics.MetricTypeNameRaw,
			katalystmetrics.MetricTag{Key: "namespace", Val: ihpa.Namespace},
			katalystmetrics.MetricTag{Key: "name", Val: ihpa.Name},
			katalystmetrics.MetricTag{Key: "type", Val: "desired"},
		)
		_ = p.metricsEmitter.StoreInt64(metricsNameIHPAReplicasMetric, int64(ihpa.Status.CurrentReplicas), katalystmetrics.MetricTypeNameRaw,
			katalystmetrics.MetricTag{Key: "namespace", Val: ihpa.Namespace},
			katalystmetrics.MetricTag{Key: "name", Val: ihpa.Name},
			katalystmetrics.MetricTag{Key: "type", Val: "current"},
		)
	}
}
...
func (p *IHPAController) syncIHPA(key string) error {
	...
	klog.InfoS("[ihpa] updated status")
	if !apiequality.Semantic.DeepEqual(ihpa.Status, ihpaCopy.Status) {
		klog.InfoS("[ihpa] updating status")
...
func (p *IHPAController) syncSPD(ihpa *apiautoscaling.IntelligentHorizontalPodAutoscaler) (*apiworkload.ServiceProfileDescriptor, error) {
	...
	if err != nil {
		...
		matched := false
		newSPD := spd.DeepCopy()
		for idx, indicator := range newSPD.Spec.ExtendedIndicator {
			if indicator.Name == resourceportrait.ResourcePortraitExtendedSpecName {
				newSPD.Spec.ExtendedIndicator[idx] = apiworkload.ServiceExtendedIndicatorSpec{
		...
		if !matched {
			newSPD.Spec.ExtendedIndicator = append(newSPD.Spec.ExtendedIndicator, apiworkload.ServiceExtendedIndicatorSpec{
				Name:       resourceportrait.ResourcePortraitExtendedSpecName,
				Indicators: runtime.RawExtension{Object: &apiconfig.ResourcePortraitIndicators{Configs: []apiconfig.ResourcePortraitConfig{*generateResourcePortraitConfig(ihpa)}}},
			})
		}

		if apiequality.Semantic.DeepEqual(spd.Spec, newSPD.Spec) {
			return spd, nil
		}

		spd, err = p.spdUpdater.UpdateSPD(p.ctx, newSPD, metav1.UpdateOptions{})
		if err != nil {
			return nil, err
		}
	}
...
func (p *IHPAController) syncVirtualWorkload(ihpa *apiautoscaling.IntelligentHorizontalPodAutoscaler) error {
	...
	klog.V(5).InfoS("[ihpa] get scaleObj", "scaleObj", scaleObj)
...
func (p *IHPAController) syncHPA(ihpa *apiautoscaling.IntelligentHorizontalPodAutoscaler, podTemplate *corev1.PodTemplateSpec) (hpa *v2.HorizontalPodAutoscaler, err error) {
	hpa, err = p.hpaLister.HorizontalPodAutoscalers(ihpa.Namespace).Get(fmt.Sprintf("%s-%s", IHPAControllerName, ihpa.Name))
	if err != nil {
		if errors.IsNotFound(err) {
